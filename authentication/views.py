...
def index(request):
    ...
    Index page view.
    """
    context = {"title": "Application Logistique pour Exploitation Agricole"}
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            return redirect("dashboard")
    context = {"title": "ALEA: Se Connecter", "form": form}
...
def account(request):
    ...
    context = {
        ...
        "employee": Employee.objects.get(user_ptr_id=request.user),
    }
    return render(request, "authentication/account.html", context)


def forgotten_password(request):
    context = {"title": "ALEA: Mot de passe oublié"}
...
def password_reset_query(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "authentication/forgotten_password.html", context
        )
...
def password_reset(request, uidb64, usr_token):
    ...
    if user is not None and token.password_token.check_token(user, usr_token):
        login(request, user)
        context = {"title": "Modification du mot de passe"}
